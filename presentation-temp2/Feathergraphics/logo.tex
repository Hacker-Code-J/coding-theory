...
\begin{document}
	...
	\begin{tikzpicture}[scale=2, line cap=round, line join=round]
		...
		%--------------------------
		\draw[thick, fill=teal!50] (0,0) circle (1.4);
...
%		decoration={
%			text along path,
%			text={|\large \bfseries|Think Globally, Act Locally}, % style & text
...
%		plot(\x, {0.5*sin(2*deg(\x r))});
		...
		\node[opacity=1] at (0,.15) {\includegraphics{logo-pic.pdf}};
%		\node[opacity=1] at (0,-.3) {\includegraphics[scale=.5]{poster-cover3.pdf}};
		% (Optional) If you want text on the bottom arc, you can do something similar
		 \path[
		   decoration={
			     text along path,
			     text={|\large \bfseries|Think Globally, Act Locally}, % for example
