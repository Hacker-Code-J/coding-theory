...
\begin{document}
	...
	The purpose of these notes is to explain, in one unified framework, the following three themes:
	...
	The basic algebraic fact is simple:
	...
	In coding theory, the quotient
	\[
	\F_q[x]/\ideal{x^n-1}
	\]
	has an additional structure: as an \(\F_q\)-vector space it is naturally identified with \(\F_q^n\), and multiplication by \(x\) becomes cyclic shift of coordinates. Because of this, ideals of the quotient ring become cyclic codes.
	...
	\section{Geometric picture of the cyclic-code ring}
...
\subsection{Closed-point versions of the coproduct picture}

The previous discussion explains the precise meaning of expressions such as
\[
...
These should be interpreted geometrically as
\[
\coprod_{\alpha\in \C}\Spec\!\bigl(\C[x]/(x-\alpha)\bigr)\to \Spec(\C[x]),
\]
and
\[
\coprod_{p\text{ prime}}\Spec\!\bigl(\Z/(p)\bigr)\to \Spec(\Z),
\]
which recover all closed points.

At the level of values, an element of the ring restricts to a section over the closed points:
\[
f \mapsto \bigl(\alpha\mapsto f(\alpha)\in \C[x]/(x-\alpha)\cong \C\bigr),
\]
and
\[
n \mapsto \bigl(p\mapsto n\bmod p \in \Z/(p)\cong \F_p\bigr).
\]

However, these closed-point pictures do \emph{not} include the generic point. The full pointwise picture requires the residue fields at \emph{all} points:
\[
\coprod_{x\in \Spec(R)} \Spec(\kk{x})\to \Spec(R).
\]
...
\begin{example}
	...
	A=\F_q[x]/(x^n-1),
	\]
	an element of \(A\) is a regular function on
	\[
	...
	a_0+a_1x+\cdots+a_{n-1}x^{n-1},
	\]
	so it also corresponds to a vector
	\[
	(a_0,a_1,\dots,a_{n-1})\in \F_q^n.
	\]
	This is the bridge from algebraic geometry to coding theory.
\end{example}
...
\begin{longtable}{@{}p{0.22\textwidth}p{0.22\textwidth}p{0.22\textwidth}p{0.26\textwidth}@{}}
	...
	Closed-point value of an element & 
	\(n\mapsto \bigl((p)\mapsto n\bmod p\bigr)\) &
	\(f(x)\mapsto \bigl((x-\alpha)\mapsto f(\alpha)\bigr)\) &
	\(\ol{a(x)}\mapsto \bigl(\mathfrak m_i\mapsto a(x)\bmod f_i(x)\bigr)\) \\
	\bottomrule
\end{longtable}
...
\begin{longtable}{@{}p{0.35\textwidth}p{0.57\textwidth}@{}}
	...
	Image description & \(C=\Im(\mu_g)\) \\
	Kernel description & \(C=\Ker(\mu_h)\) \\
	Element of \(A\) as a closed-point function & 
	\(\ol{a(x)}\mapsto \bigl(\mathfrak m_i\mapsto a(x)\bmod f_i(x)\in \F_q[x]/(f_i)\bigr)\) \\
	\bottomrule
\end{longtable}
...
\[
\widetilde f:\Spec(\C[x])\longrightarrow \coprod_{x\in \Spec(\C[x])}\kappa(x),
\qquad
x\longmapsto f(x)\in \kappa(x).
\]
Here
\[
...
\[
\widetilde n:\Spec(\Z)\longrightarrow \coprod_{x\in \Spec(\Z)}\kappa(x),
\qquad
x\longmapsto n(x)\in \kappa(x),
\]
where
\[
...
\newpage
\subsection{The closed-point evaluation section for the cyclic-code ring}
...
\]
Hence in that case an element of \(A\) is literally a tuple
\[
...
a_0+a_1x+\cdots+a_{n-1}x^{n-1},
\]
so as an \(\F_q\)-vector space,
\[
A\cong \F_q^n.
\]
Under this identification, multiplication by \(x\) becomes cyclic shift of coordinates. Therefore the ideals of \(A\) are exactly the cyclic codes of length \(n\) over \(\F_q\).
...
In the previous sections, we interpreted an element \(a\in R\) as a pointwise assignment
\[
x\longmapsto a(x)\in \kk{x},
\qquad x\in \Spec(R),
\]
...
f(p)\in \mathbb{R}.
\]
This is a section of the trivial line bundle of scalar-valued functions.
...
v(p)\in T_pM.
\]
Thus a vector field is a section of the tangent bundle.

So already in differential geometry we distinguish two kinds of sections:
...
A vector field is therefore something that assigns, at each point \(x\), an element of \(T_xX\), just as a regular function assigns an element of \(\kk{x}\).

So pointwise, one has the following comparison:
...
\subsection{Example: the cyclic-code ring}
...
As explained earlier, an element \(\ol{a(x)}\in A\) is a regular function on \(X\), that is, a section of \(\mathcal O_X\). Pointwise it assigns values
\[
y\longmapsto \ol{a(x)}(y)\in \kk{y},
\qquad y\in X.
\]
...
If \(x^n-1\) is squarefree, then \(X\) is a finite reduced scheme, so its tangent spaces at closed points vanish. If \(x^n-1\) has repeated irreducible factors, then \(X\) is nonreduced, and tangent spaces may be nonzero. Thus tangent spaces again detect the nilpotent structure that is invisible at the level of the underlying set of points.

\subsection{Summary dictionary}

For a scheme \(X=\Spec(R)\), one should compare:

\[
\begin{array}{c|c}
	\text{object} & \text{fiber over }x \\
	\hline
	\text{regular function} & \kk{x} \\
	\text{vector field} & T_xX
\end{array}
...
\[
\boxed{
	\text{sections discussed earlier are sections of }\mathcal O_X,
	\text{ not of the tangent bundle.}
}
\]

The tangent-bundle analogue is obtained by replacing the residue-field value \(a(x)\in \kk{x}\) with a tangent vector \(v(x)\in T_xX\).
