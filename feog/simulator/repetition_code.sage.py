...
# This file was *autogenerated* from the file repetition_code.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0p1 = RealNumber('0.1'); _sage_const_1000 = Integer(1000); _sage_const_10000 = Integer(10000); _sage_const_25 = Integer(25); _sage_const_5000 = Integer(5000); _sage_const_5 = Integer(5); _sage_const_0p2 = RealNumber('0.2'); _sage_const_0p50 = RealNumber('0.50'); _sage_const_0p02 = RealNumber('0.02'); _sage_const_58 = Integer(58); _sage_const_7 = Integer(7); _sage_const_0p08 = RealNumber('0.08'); _sage_const_0p10 = RealNumber('0.10'); _sage_const_3 = Integer(3); _sage_const_9 = Integer(9)# ============================================================
...
class RepetitionCodeSimulator:
    def __init__(self, n):
        ...
        self.n = ZZ(n)
        self.k = ZZ(_sage_const_1 )
        self.F = GF(_sage_const_2 )
        self.t = (self.n - _sage_const_1 ) // _sage_const_2 

    def _check_bit(self, b):
        if int(b) not in [_sage_const_0 , _sage_const_1 ]:
    ...
    def _to_vector(self, word):
        ...
        cleaned = []
        for x in word:
            if int(x) not in [_sage_const_0 , _sage_const_1 ]:
    ...
    def random_error_vector(self, p):
        ...
        return vector(self.F, [_sage_const_1  if random() < p else _sage_const_0  for _ in range(self.n)])
    ...
    def decode_word(self, received_word):
        ...
        zeros = self.n - ones
        return _sage_const_1  if ones > zeros else _sage_const_0 
    ...
    def theoretical_block_error_probability(self, p):
        self._check_probability(p)
        total = _sage_const_0 
        for i in range(self.t + _sage_const_1 , self.n + _sage_const_1 ):
            total += binomial(self.n, i) * (p**i) * ((_sage_const_1  - p)**(self.n - i))
        return RR(total)

    def demo_single_trial(self, bit=None, p=_sage_const_0p1 ):
        self._check_probability(p)
        if bit is None:
            bit = randint(_sage_const_0 , _sage_const_1 )
        self._check_bit(bit)
    ...
    def simulate(self, bit=None, p=_sage_const_0p1 , trials=_sage_const_1000 , verbose=False):
        self._check_probability(p)
        if trials <= _sage_const_0 :
            raise ValueError("trials must be a positive integer.")

        errors = _sage_const_0 
        total_channel_flips = _sage_const_0 

        for _ in range(trials):
            b = randint(_sage_const_0 , _sage_const_1 ) if bit is None else int(bit)
            c = self.encode_bit(b)
            y, e = self.transmit(c, p)
            b_hat = self.decode_word(y)
    ...
    def compare_with_theory(self, bit=None, p=_sage_const_0p1 , trials=_sage_const_10000 ):
    ...
    def transmit_message(self, bits, p=_sage_const_0p1 , verbose=True):
        decoded = []
        records = []
        for b in bits:
            c = self.encode_bit(b)
        ...
        if verbose:
            ...
            for i, rec in enumerate(records, start=_sage_const_1 ):
                b, c, e, y, bhat = rec
    ...
    def plot_multiple_theory(n_values, p_values):
        ...
        for n in n_values:
            ...
            G += list_plot(pts, plotjoined=True, thickness=_sage_const_2 , legend_label=f"n={n}")
        G.axes_labels([r"$p$", r"$P_{\mathrm{block\ error}}$"])
        G.set_legend_options(loc='upper left')
        return G

    @staticmethod
    def comparison_table(n_values, p=_sage_const_0p1 , trials=_sage_const_5000 ):
        rows = []
        for n in n_values:
            sim = RepetitionCodeSimulator(int(n))
...
def read_odd_n(default=_sage_const_5 ):
    raw = input(f"Enter odd repetition length n [default {default}]: ").strip()
    if raw == "":
        return ZZ(default)
    n = ZZ(raw)
    if n <= _sage_const_0  or n % _sage_const_2  == _sage_const_0 :
        raise ValueError("n must be a positive odd integer.")
    return n

def read_probability(default=_sage_const_0p2 ):
    ...
    while True:
        try:
            ...
            if choice == "0":
                ...
                n = read_odd_n(_sage_const_5 ); p = read_probability(_sage_const_0p2 ); bit = read_bit_or_random()
                RepetitionCodeSimulator(n).demo_single_trial(bit=bit, p=p)
                pause()
            elif choice == "2":
                n = read_odd_n(_sage_const_5 ); p = read_probability(_sage_const_0p2 ); trials = read_trials(_sage_const_10000 ); bit = read_bit_or_random()
                ...
                n = read_odd_n(_sage_const_7 ); p = read_probability(_sage_const_0p08 ); bits = read_message("1,0,1,1,0")
                RepetitionCodeSimulator(n).transmit_message(bits, p=p, verbose=True)
                pause()
            elif choice == "5":
                n = read_odd_n(_sage_const_5 ); p_values = read_p_values(_sage_const_0p50 , _sage_const_0p02 )
                G = RepetitionCodeSimulator(n).plot_theory(p_values)
                show(G)
                ask_save_plot(G, default_base=f"repetition_n{n}_theory")
                pause()
            elif choice == "6":
                p = read_probability(_sage_const_0p10 ); trials = read_trials(_sage_const_5000 )
                show(RepetitionCodeSimulator.comparison_table([_sage_const_1 ,_sage_const_3 ,_sage_const_5 ,_sage_const_7 ,_sage_const_9 ], p=p, trials=trials))
                pause()
            elif choice == "7":
                p_values = read_p_values(_sage_const_0p50 , _sage_const_0p02 )
                G = RepetitionCodeSimulator.plot_multiple_theory([_sage_const_1 ,_sage_const_3 ,_sage_const_5 ,_sage_const_7 ,_sage_const_9 ], p_values)
                show(G)
